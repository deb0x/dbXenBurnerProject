...
export function MintDbXeNFT(): any {
  ...
  const datePolygon: any = new Date(Date.UTC(2030, 12, 17, 17, 48, 8, 0));
  const dateAvax: any = new Date(Date.UTC(2030, 12, 17, 14, 17, 12, 0));
  const dateBsc: any = new Date(Date.UTC(2030, 12, 17, 14, 32, 44, 0));
  const dateFantom: any = new Date(Date.UTC(2030, 12, 13, 14, 8, 55, 0));
  const dateBase: any = new Date(Date.UTC(2030, 12, 13, 14, 57, 43, 0));
  const dateETH: any = new Date(Date.UTC(2030, 12, 13, 14, 11, 11, 0));
  const dateOP: any = new Date(Date.UTC(2030, 12, 13, 14, 46, 53, 0));
  const dateEVMOS: any = new Date(Date.UTC(2030, 12, 13, 16, 3, 25, 0));
  const dateGLMR: any = new Date(Date.UTC(2030, 12, 13, 14, 36, 6, 0));
  const dateETHPOW: any = new Date(Date.UTC(2030, 12, 13, 14, 2, 43, 0));
  const datePLS: any = new Date(Date.UTC(2030, 12, 13, 14, 11, 25, 0));
  const now: any = Date.now();
  const [displayGaslessClaim, setDisplayGaslessClaim] = useState<boolean>(
    false
  );
  ...
  useEffect(() => {
    if (chain.chainId == "80001") {
      preferredRelays = ["https://mumbai.v3.relays.bwl.gg/"];
  ...
  useEffect(() => {
    startMoralis();
    getXENFTs();
    isApprovedForAll().then((result: any) =>
      result ? setXeNFTWrapApproved(true) : setXeNFTWrapApproved(false)
    );
    currentCycleTotalPower();
  }, [chain, account]);

  useEffect(() => {
    if (xeNFTWrapped) getXENFTs();
  }, [xeNFTWrapped]);

  const startMoralis = () => {
    if (!Moralis.Core.isStarted) {
      Moralis.start({
        apiKey: process.env.REACT_APP_MORALIS_KEY_NFT,
  ...
  async function initProvider() {
    ...
    const gsnp = RelayProvider.newProvider({
      provider: web3Provider,
      config: configTokenPaymaster,
    });
    await gsnp.init();

    const etherProviderTP = new ethers.providers.Web3Provider(
      (gsnp as any) as providers.ExternalProvider
    );

    let xenC = new ethers.Contract(
      chain.xenftAddress,
      abi,
      etherProviderTP.getSigner(0)
    );
          ...
          dbxenftFactory.lastCycleReward().then((result: any) => {
            setCurrentRewardPower(
              ethers.utils.formatEther(
                result.add(result.div(ethers.utils.parseEther("1")))
              )
            );
          });
        } else {
  ...
  const getXENFTs = () => {
    ...
    if (Number(chain.chainId) == 369) {
      ...
      getNFTsWithRPC(account ? account : "", XENFTContract, 0, 10).then(
        async (result) => {
        ...
        if (Number(chain.chainId) == 9001) {
          ...
          if (Number(chain.chainId) == 1284) {
            ...
            if (Number(chain.chainId) == 10) {
              getNFTsOnOP(account ? account : "", chain.xenftAddress).then(
                async (results) => {
                      ...
                      if (result.rawMetadata?.attributes?.length != undefined) {
                        if (result.rawMetadata?.attributes != null) {
                          const maturityDateObject = result.rawMetadata?.attributes.find(
                            (item: { trait_type: string }) =>
                              item.trait_type == "Maturity DateTime"
                          );
                          resultAttributes = result.rawMetadata.attributes;
                          const maturityDate = new Date(
                            maturityDateObject.value
                          );
                          let xenEstimated = await getNFTRewardInXen(
                            Number(maturityDate) / 1000,
                            Number(resultAttributes[1].value),
                            resultAttributes[4].value,
                            resultAttributes[8].value,
                            resultAttributes[3].value,
                            resultAttributes[2].value
                          );
                          try {
                            xenftEntries.push({
                              id: +result.tokenId,
                              claimStatus: getClaimStatus(
                                resultAttributes[5].value,
                              ...
                              estimatedXen: ethers.utils.formatEther(
                                xenEstimated
                              ),
              ...
              if (Number(chain.chainId) == 8453) {
                ...
                getWalletNFTsForUser(
                  ...
                  if (cursor != null) {
                    while (cursor != null) {
                      let newPage: any = await getWalletNFTsForUser(
                        chain.chainId,
                        chain.xenftAddress,
                        cursor
                      );
                      ...
                      if (
                        newPage.result?.length != 0 &&
                        newPage.result != undefined
                      ) {
                  ...
                  if (resultArray?.length != 0 && resultArray != undefined) {
                    ...
                    for (let i = 0; i < resultArray?.length; i++) {
                      ...
                      if (result != undefined) {
                        ...
                        let nftImage;
                        if (
                          result?.normalized_metadata?.attributes === null ||
                          result?.normalized_metadata?.attributes.length ===
                            0 ||
                          result.metadata === null
                        ) {
                          let resultFromCall = await getNFTsWithRPCForMoralisChain(
                            result.token_id,
                            XENFTContract
                          );
                          resultAttributes = resultFromCall.attributes;
                          nftImage = resultFromCall.image;
                        } else {
                          resultAttributes =
                            result.normalized_metadata.attributes;
                          nftImage = result.normalized_metadata.image;
                        }
  ...
  async function putDataOnRpcChains(result: any) {
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      let xenEstimated = await getNFTRewardInXen(
        Number(maturityDate) / 1000,
        Number(result[i].attributes[1].value),
        result[i].attributes[4].value,
        result[i].attributes[8].value,
        result[i].attributes[3].value,
        result[i].attributes[2].value
      );
      xenftEntries.push({
        id: +result[i].token_id,
        claimStatus: getClaimStatus(
          result[i].attributes[5].value,
          result[i].attributes[7].value
        ),
  ...
  function getClaimStatus(resultAttributes5: any, resultAttributes7: any) {
    let formattedMaturityDate = new Date(resultAttributes5 + resultAttributes7);
  ...
  async function addXENFTRedeemedStatus(xenftArray: any) {
    ...
    for (let i = 0; i < arrayLen; i++) {
      calls.push({
        reference: "getMintInfo",
        methodName: "mintInfo",
        methodParameters: [xenftArray[i].token_id],
      });
    }
    ...
    const response: ContractCallResults = await multicall.call(
      contractCallContext
    );
    ...
    for (let i = 0; i < arrayLen; i++) {
      calls2.push({
        reference: "isRedeemed",
        methodName: "getRedeemed",
        methodParameters: [
          response.results.XENTorrent.callsReturnContext[i].returnValues[0],
        ],
      });
    }
    ...
    const response2: ContractCallResults = await multicall.call(
      contractCallContext2
    );

    for (let i = 0; i < arrayLen; i++) {
      xenftArray[i].redeemed =
        response2.results.MintInfo.callsReturnContext[i].returnValues[0];
    }
  ...
  async function getWalletNFTsForUser(
    chain: any,
    nftAddress: any,
    cursor: any
  ) {
  ...
  async function getNFTsOnOP(accountAddress: any, nftAddress: any) {
    ...
    while (pageNumber > 0) {
      ...
      let data: any = await alchemy.nft.getNftsForOwner(
        accountAddress,
        options
      );
  ...
  async function getNFTsWithRPC(
    ...
    if (startIndex == 0 && endIndex == 10) {
      ...
      if (tokenIds.length < 10) endIndex = tokenIds.length;
  ...
  async function getNFTsWithRPCForMoralisChain(
    toeknId: any,
    XENFTContract: any
  ) {
    let base64Data = await XENFTContract.tokenURI(Number(toeknId));
    const dataWithoutPrefix = base64Data.split(",")[1];
    const decodedData = atob(dataWithoutPrefix);
    const decodedObject = JSON.parse(decodedData);
    return {
      attributes: decodedObject.attributes,
  ...
  async function isApprovedForAll() {
    const xenftContract = XENFT(library, chain.xenftAddress);
    const isApprovedForAll = await xenftContract.isApprovedForAll(
      account,
      chain.dbxenftFactoryAddress
    );
  ...
  async function approveForAll() {
    ...
    try {
      const tx = await xenftContract.setApprovalForAll(
        chain.dbxenftFactoryAddress,
        true
      );
      tx.wait()
        .then((result: any) => {
          setNotificationState({
            message:
              "Your succesfully approved contract for handling your XENFTs.",
            open: true,
            severity: "success",
          });
        ...
        .catch((error: any) => {
          setNotificationState({
            message: "Contract couldn't be approved for handling your XENFTs!",
            open: true,
            severity: "error",
          });
          setLoadingApprove(false);
      ...
      setNotificationState({
        message:
          "You rejected the transaction. Contract hasn't been approved for burn.",
        open: true,
        severity: "info",
      });
      setLoadingApprove(false);
  ...
  async function mintDBXENFT(
    ...
    try {
      if (Number(chain.chainId) == 369) {
        fee = await calcMintFeePLS(maturityTs, VMUs, EAA, term, AMP, cRank);
        gasLimitForTransaction = BigNumber.from("3500000");
      } else {
        if (Number(chain.chainId) == 1) {
          fee = await calcMintFeeETH(maturityTs, VMUs, EAA, term, AMP, cRank);
      ...
      await tx
        ...
        .then(async (result: any) => {
          for (let i = 0; i < result.events.length; i++) {
            if (result.events[i].event == "DBXeNFTMinted") {
              ...
              writePerCycle(
                Number(result.events[i].args.DBXENFTId),
                maturityTs,
                Number(chain.chainId)
              );
              setNotificationState({
                message: "Your succesfully minted a DBXENFT.",
                open: true,
                severity: "success",
              });
              setLoading(false);
        ...
        .catch((error: any) => {
          setNotificationState({
            message: "Contract couldn't mint your DBXENFT!",
            open: true,
            severity: "error",
          });
          setLoading(false);
  ...
  function calcMaturityDays(term: any, maturityTs: any) {
    ...
    if (currentTimestamp < maturityTs) {
      daysTillClaim = Math.floor(
        (maturityTs - currentTimestamp) / SECONDS_IN_DAYS
      );
      daysSinceMinted = term - daysTillClaim;
    } else {
      daysSinceMinted = Math.floor(
        (term * SECONDS_IN_DAYS + (currentTimestamp - maturityTs)) /
          SECONDS_IN_DAYS
      );
    }
  ...
  async function calcMintFee(
    ...
    const xenMulReduction = estReward
      .mul(BigNumber.from(maxPctReduction))
      .div(BigNumber.from(10_000_000));
    const minFee = BigNumber.from(1e15);
    const rewardWithReduction = xenMulReduction.div(
  ...
  async function calcMintFeePLS(
    ...
    const fee = minFeeBigNumber.gt(rewardWithReductionBigNumber)
      ? minFeeBigNumber
      : rewardWithReductionBigNumber;
    return fee.add(fee.div(10));
  }
  ...
  async function getNFTRewardInXen(
    ...
    const cRankDelta = Math.max(
      globalRank.sub(BigNumber.from(cRank)).toNumber(),
      2
    );
    ...
    return BigNumber.from(rew)
      .mul(BigNumber.from(VMUs))
  ...
  function calcPenalty(secsLate: number) {
    ...
    const penalty = BigNumber.from(1)
      .shl(daysLate + 3)
  ...
  async function calcTxCost(xenftId: number) {
    ...
    const xenQuote = ethers.utils.formatEther(
      await QuoterContract._getQuote(
        txCost,
        chain.UniPoolXen,
        chain.token1,
        chain.token2,
    ...
    setServiceCost(Math.floor(parseInt(xenQuote) * 0.2));
  ...
  const handleExpandRow = (id: any) => {
    XENFTs.map((data: any) => {
      if (id == data.id) {
        ...
        if (displayDbxenftDetails == false) setDisplayDbxenftDetails(true);
      }
    });
  };
    ...
    axios.request(options).then(async (result) => {
      if (result.data.result != undefined) {
        if (chainForGas.includes(Number(chain.chainId))) {
          ...
          price = Number(
            web3.utils.fromWei(result.data.result.toString(), "Gwei")
          );
    ...
    if (chainsForPagination.includes(Number(chain.chainId))) {
      ...
      if (XENFTs.length > startIndex) {
        ...
        if (allXENFTs?.length !== 0 && allXENFTs !== undefined) {
          let lastIndex = startIndex + rowsPerPage;
          if (lastIndex > allXENFTs.length) lastIndex = allXENFTs.length;
          if (Number(chain.chainId) == 8453) {
            getNFTsWithRPC(
              account ? account : "",
              XENFTContract,
  ...
  const handleChangeRowsPerPage = (
    ...
    if (
      ...
      const newArray = XENFTs.slice(0, 0)
        .concat(currentPageContent)
  ...
  async function handleClaim(xenftId: any) {
    try {
      ...
      await tx
        .wait()
        .then(async (result: any) => {
          const submissionDetails = await gsnProvider._getSubmissionDetailsForRelayRequestId(
            tx.hash
          );
          const transactionHash = await gsnProvider._getTransactionIdFromRequestId(
            tx.hash,
            submissionDetails
          );
          ...
          if (chain.chainId == "80001") {
            setNotificationState({
              message: `Transaction success! Check it out in the <a href="https://mumbai.polygonscan.com/tx/${transactionHash.toString()}" target="_blank" rel="noopener noreferrer" className="alert-link">explorer</a>.`,
              open: true,
              severity: "success",
            });
          } else {
            setNotificationState({
              message: `Transaction success! Check it out in the <a href="https://polygonscan.com/tx/${transactionHash.toString()}" target="_blank" rel="noopener noreferrer" className="alert-link">explorer</a>.`,
              open: true,
              severity: "success",
            });
          }
      ...
      if (
        ...
        if (error.message.includes("Reward less than tx cost")) {
          setNotificationState({
            message: `Reward less than tx cost`,
            open: true,
            severity: "error",
          });
        }
        ...
        setNotificationState({
          message: `XENFT claim failed. Check console for details.(ctrl + shift + i)`,
          open: true,
          severity: "error",
        });
      }
  ...
  const handleAlreadyRedeemed = async (data: any) => {
    ...
    if (redeemed) {
      setNotificationState({
        message: `This XENFT has already been redeemed.`,
        open: true,
        severity: "warning",
      });
      setTimeout(() => setNotificationState({}), 5000);
  ...
  return (
    <div
      className={`content-box content-box-table ${
        initLoading ? "loading" : ""
      }`}
    >
                              ...
                              <div className="col xenft-container">
                                <div className="xenft-details">
                                  ...
                                  <div className="details-container">
                                    <div className="row">
                                      <div className="col-6">
                                        <p className="label">Protocol fee</p>
                                        <p className="value">
                                          {Number(
                                            DBXENFT?.protocolFee
                                    ...
                                    <div className="row">
                                      <div className="col-6">
                                        ...
                                        <p className="value">0x0a25…fa59</p>
                                      </div>
                                      <div className="col-6">
                                ...
                                {isRedeemed ? (
                                  <p className="mb-3">
                                    This XENFT is redeemed. You will get a power
                                    of 1.
                                  </p>
                                ) : (
                                  <p className="mb-3">
                                    Your Base Power will be calculated at the
                                    end of the cycle.
                                  </p>
                                )}
                                <div className="burn-button-container">
                                  <LoadingButton
                                    ...
                                    type="button"
                                    disabled={
                                      data.claimStatus === "Blackout period"
                                    }
                        ...
                        <td colSpan={12}>
                          ...
                          <div className="details-container">
                            <div className="reward-row">
                              <div className="reward-details">
                                ...
                                <p className="value">{data.estimatedXen} XEN</p>
                              </div>
                              <div className="reward-details">
                                <p className="label">Transaction cost</p>
                                <p className="value">-{txCost} XEN</p>
                              </div>
                              <div className="reward-details">
                                <p className="label">Service fee:</p>
                                <p className="value">-{serviceCost} XEN</p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="reward-row">
                            <button
                              ...
                              type="button"
                              disabled={
                                data.estimatedXen - txCost - serviceCost < 0
                              }
                              onClick={() => {
                                handleClaim(data.id);
                              }}
            ...
            <div className="card">
              ...
              <div className="card-body">
                ...
                <p className="card-text">
                  ...
                  We now have the final version of the lite paper available
                  here.
                </p>
                <a
                  href="https://dbxenft-litepaper.gitbook.io/dbxenft/"
                  target="_blank"
                  className="btn"
                >
                  Read the document
