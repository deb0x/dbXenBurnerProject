...
describe("Test fee claiming for users/frontends", async function() {
    let rewardedAlice, rewardedBob, rewardedCarol, frontend, dxnERC20, deb0xViews;
    let alice, bob;
    ...
    it("should send messages with allice and bob and claim fees after cycle0 but we have a difference", async() => {
        ...
        // tesing rewards 
        let frontDBX = await dxnERC20.balanceOf(feeReceiver.address);
        await frontend.claimClientRewards();
        frontDBX = await dxnERC20.balanceOf(feeReceiver.address);
    ...
    it("should test getting rewards with allice sending 3 messages and bob 8 to see if any diff", async() => {
        ...
        let aliceBal = await dxnERC20.balanceOf(alice.address);
        let bobBal = await dxnERC20.balanceOf(bob.address);
